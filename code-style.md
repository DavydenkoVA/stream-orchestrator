...
Важно включить все правила, т.к. по-умолчанию включено малое количество. А так же после этого имеет смысл ряд правил исключить.
Они описаны по схеме «правило» — «почему его отключили» (учитывайте, что не все отключаются глобально, некоторые только для тестов):
