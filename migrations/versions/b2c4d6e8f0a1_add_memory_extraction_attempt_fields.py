...
def upgrade() -> None:
    """Upgrade schema."""
    op.add_column('chat_messages', sa.Column('memory_process_attempts', sa.Integer(), nullable=False, server_default='0'))
    ...
    op.create_index(op.f('ix_chat_messages_memory_last_attempt_at'), 'chat_messages', ['memory_last_attempt_at'], unique=False)
    op.create_index(op.f('ix_chat_messages_memory_last_error_code'), 'chat_messages', ['memory_last_error_code'], unique=False)
