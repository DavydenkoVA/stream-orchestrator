...
class PromptStore:
    ...
    def _resolve_path(self, name: str) -> Path:
        candidate: typing.Final = (self.base_dir / name).resolve()
        base: typing.Final = self.base_dir.resolve()
        if candidate != base and base not in candidate.parents:
            raise ValueError("Prompt path is outside prompt storage")
        return candidate

    def read_raw(self, name: str) -> str:
        path: typing.Final = self._resolve_path(name)
        if not path.exists():
    ...
    def _extract_required_fields(self, template: str, *, template_name: str) -> set[str]:
        fields: typing.Final[set[str]] = set()
        try:
    ...
    def get_required_fields(self, name: str) -> set[str]:
        template: typing.Final = self.read(name)
